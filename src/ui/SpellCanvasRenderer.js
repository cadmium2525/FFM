...
const referenceLockedOptions = ({
  ...
  reference: {
    sourceCitation,
    sourceMediaHash: `sha256:evidence-frame-set:${mediaHash}`,
    captureId,
    region: 'JP',
...
const timeReferenceSequence = ({
  ...
  originalEffectHeader,
  family,
  resultPolicy = 'final-impact',
  placement = 'centroid',
}) => sequence(frameCount, impactFrames, phases, {
  ...referenceLockedOptions({
    captureId: `yt--2avOoXFONk-${sceneId}-${frames[0].seconds}-${frames.at(-1).seconds}`,
    sourceCitation: 'https://www.youtube.com/watch?v=-2avOoXFONk',
    mediaHash,
    frames,
    mode: sceneId === 'meteor'
      ? 'full-stage-sfc-4x3-to-portrait'
      : sceneId === 'return'
        ? 'target-pixel-dissolve-and-blackout-sfc-4x3-to-portrait'
        : 'caster-target-sfc-4x3-to-portrait',
    placement,
    sceneSpace: 'stage',
    renderMode: 'stage-direct',
  }),
...
const SPELL_PIXEL_SEQUENCES = Object.freeze({
  ...
  haste: timeReferenceSequence({
    sceneId: 'haste', mediaHash: 'efde18707ce7efd608531dcd5b16c8b81ded7dacdcdc05e23392267435f5d6d0', frameCount: 96, impactFrames: [62], originalEffectHeader: '2A 10 2D 00 6B', family: 'haste-header-2a-10-2d-00-6b',
    phases: [phase('target-white-radial-seed', 0, 22), phase('silver-shard-fan', 23, 47), phase('orange-core-four-satellites', 48, 61), phase('haste-latch', 62, 72), phase('decay', 73, 95)],
    frames: [
      { role: 'cast', seconds: 25, sha256: 'be9050a705a84106ecc9ac45bacf9d252ab4e16234f9595822a8c717e4a763d9' },
      { role: 'development', seconds: 26, sha256: '24f920bf57c1e3874d58db0d5c537a66d33418f1b6dd9c73c205b19ddcb9c494' },
      { role: 'impact', seconds: 27, sha256: 'd999b95dc1cb04dea17ecf90ebd48d35196e08b95d08548f7ace79399786d6e4' },
      { role: 'decay', seconds: 28, sha256: '5318bbe7bac5d857512a1125193175d4feb5919cca5de64263710d72e4619f6d' },
    ],
  }),
  ...
  meteor: timeReferenceSequence({
    sceneId: 'meteor', mediaHash: 'daa33ed4a806f72108c604fb49affd5090d785e893bbb036f7ee4fc3063c359f', frameCount: 252, impactFrames: [98, 126, 154, 182], originalEffectHeader: '04 10 7B 91 46', family: 'time-magic-script-7b', resultPolicy: 'split-amount',
    phases: [phase('caster-white-radial-seed', 0, 31), phase('red-black-palette-field', 32, 71), phase('fireball-descent', 72, 97), phase('four-hit-barrage', 98, 197), phase('red-black-afterfield', 198, 227), phase('decay', 228, 251)],
    frames: [
      { role: 'cast', seconds: 99, sha256: '35bca98ec1ff7ddeb2a779bf2e983bb0a770e982bc108bebeda751557d21ea83' },
      { role: 'development', seconds: 100, sha256: '78b7d39b659c6436497feb1c7102ba04ec7c8b3930acb748fc38aa2f1694f9eb' },
      { role: 'impact-a', seconds: 101, sha256: '82d3d5b76f1c05ac8c62ee8b55ad8d4df1285b2cf5cbc756119c2bca2ce9e157' },
      { role: 'impact-b', seconds: 102, sha256: '1092ff2e9d4ba7b500862f0f2377e05b64afcbb3eaa1a0d0916d2ba28a3b3c35' },
      { role: 'decay', seconds: 103, sha256: '3d6c572fab3b79d6a8de5f1f31755092234548900fd90ef2db0785d602d02bef' },
    ],
  }),
  gravity: timeReferenceSequence({
    sceneId: 'gravity', mediaHash: 'df0c0a76c07dcbf3c12086d61c080c40f1a81993db675c5014b7383caef7757f', frameCount: 108, impactFrames: [74], originalEffectHeader: '2E 25 7A 00 2D', family: 'time-magic-script-7a',
    phases: [phase('caster-white-radial-seed', 0, 22), phase('indigo-orb-launch', 23, 50), phase('target-orb-capture', 51, 73), phase('white-lightning-latch', 74, 88), phase('decay', 89, 107)],
    frames: [
      { role: 'cast', seconds: 38, sha256: 'ff4cd782d6f4737db75b8486553992237873c0598a1c913fda67c5b7e280d88a' },
      { role: 'development', seconds: 39, sha256: '5b4d03f8f93d4e8a1be7b2f5e6ee659097c8656c2f2f9eec7e300b5a2ae30a87' },
      { role: 'impact', seconds: 40, sha256: '833434703e85595567577a8d64a976bce6c7825705edd3c5466856de3bd2591b' },
      { role: 'decay', seconds: 41, sha256: '75ca0aa90a6bdb6ff66b6563d64cd6cfe40f186dd893c2f52790b8a206affbf6' },
    ],
  }),
  ...
  return: timeReferenceSequence({
    sceneId: 'return', mediaHash: '6d9c605bf817f99c6ee82988fb54498452f5a98f71e87a8696b29b6713c5edb3', frameCount: 168, impactFrames: [110], originalEffectHeader: '00 00 75 00 08', family: 'time-magic-script-75',
    phases: [phase('caster-white-radial-seed', 0, 27), phase('target-pixel-expansion', 28, 73), phase('stage-block-fracture', 74, 109), phase('timeline-reset-blackout', 110, 129), phase('decay', 130, 167)],
    frames: [
      { role: 'cast', seconds: 73, sha256: '9f92cb8168871b60b163082cb39d5dea0503a89f489b708f94fe5e2cf05556bf' },
      { role: 'development', seconds: 75, sha256: 'c8ff164f336362638dba607f4355ffe34dd67ab105ff8a8c19bb200300784e25' },
      { role: 'impact', seconds: 76, sha256: 'f203e159bbd1b3bb4d33d17feeb6c961ed61eb578753098748325cae01efc10e' },
      { role: 'decay', seconds: 77, sha256: 'bbd05efb2c78aacb7e2599eb0689b8bc3cd80c29f8a85d84057f072903a1142f' },
    ],
  }),
  missile: sequence(72, [54], [
    phase('white-orb-launch', 0, 11),
    phase('crescent-echo-flight', 12, 41),
    phase('orange-blue-target-flicker', 42, 53),
    phase('result-latch', 54, 61),
    phase('decay', 62, 71),
  ], referenceLockedOptions({
    captureId: 'yt-P0xgkoSS4fE-missile-50.778-52.084',
    sourceCitation: 'https://www.youtube.com/watch?v=P0xgkoSS4fE&t=50s',
    mediaHash: 'd4dd0d6091b48b0f8df1cba29467129181bed8161b181c1157027ff63d5f91ff',
    mode: 'caster-to-target-4x3-to-portrait',
    frames: [
      { role: 'cast', seconds: 50.777721, sha256: '50b5040d7733d6853e82e71c8f987ebca47268e0aab011292dbb9b0e1bb7acfb' },
      { role: 'development', seconds: 51.197184, sha256: '7ceb6dce2e028036df1fee6ab10ac1788aaedd19c2a45ee33c8085bd3a83cba0' },
      { role: 'impact', seconds: 51.596979, sha256: 'd88e7946b5638a5523c59183579e5134928bc322b1875116c2ab3fa698697554' },
      { role: 'decay', seconds: 52.083692, sha256: 'fe4b884843fcc6d2f6c809898487493d9fde6e626eafa96904ae22c493917df4' },
    ],
  })),
  flare: sequence(408, [372], [
    phase('green-caster-aura', 0, 47),
    phase('red-black-orb-field', 48, 119),
    phase('target-gold-ignition', 120, 189),
    phase('white-hot-sphere', 190, 264),
    phase('red-black-afterflash', 265, 371),
    phase('damage-latch', 372, 383),
    phase('decay', 384, 407),
  ], referenceLockedOptions({
    captureId: 'yt-R-1l6OvnCVk-ff5-flare-20.404-27.168',
    sourceCitation: 'https://www.youtube.com/watch?v=R-1l6OvnCVk&t=20s',
    mediaHash: '2ad4be66f44b67cff4231f1532298b3846ebd04f06614bbb573792c75408f961',
    mode: 'full-stage-target-anchored-4x3-to-portrait',
    frames: [
      { role: 'cast', seconds: 20.403715, sha256: 'd1d9f729704df8c909fc8b931fd0d1181ec3a1b800846fa02eaf199fd108a67a' },
      { role: 'development', seconds: 21.535541, sha256: '2807b67d5997093f72ce5555ae411b07d71e46717727ffa39bd0834f14fb4479' },
      { role: 'impact', seconds: 24.425791, sha256: 'a97b982e222f4a726ff3610a7890796011a3bc49342bcc7af0ae374560bc4b20' },
      { role: 'decay', seconds: 27.168281, sha256: '4061e41463e6ac223a17feb866739fa356865be38bd06621f7ba53cfc03fd3fd' },
    ],
  })),
  'level-5-death': sequence(180, [126], [
    phase('green-serrated-sweep', 0, 55),
    phase('target-ring-repeat', 56, 91),
    phase('cream-orange-success-bursts', 92, 125),
    phase('status-latch', 126, 143),
    phase('decay', 144, 179),
  ], referenceLockedOptions({
    captureId: 'yt-uFbq-WKOJHo-level5death-16.174-18.929',
    sourceCitation: 'https://www.youtube.com/watch?v=uFbq-WKOJHo&t=16s',
    mediaHash: 'e0fe39414fd4e0ec0b0748063609364e1b42a4769a0f4e29190701a85cfbbec8',
    mode: 'multi-target-4x3-to-portrait',
    frames: [
      { role: 'cast', seconds: 16.173912, sha256: 'bc532cada671ff1bfb75685870708c2b08715a5ba07294b0dc875debbd28acfa' },
      { role: 'development', seconds: 16.759155, sha256: 'dab903fdcc7f436f566e6886915e34f5c496c5026cab2b9385a6c00891333a19' },
      { role: 'impact', seconds: 17.842635, sha256: 'f535f11a0b2121ad4829b9ddcd632924c01ce9d34fa30bf1213ce62455209a8b' },
      { role: 'decay', seconds: 18.928904, sha256: '6e89ac2911460e5d839164dd20e920f71e0df9f05780b0d9aa1b3205ee17f092' },
    ],
  })),
  ...
  hastega: sequence(96, [62], [phase('target-white-radial-seed', 0, 22), phase('silver-shard-fan', 23, 47), phase('orange-core-four-satellites', 48, 61), phase('haste-latch', 62, 72), phase('decay', 73, 95)], { placement: 'centroid', sceneSpace: 'stage', renderMode: 'stage-direct', originalEffectHeader: '2A 10 2D 00 6B', sharedOriginalFamily: 'haste-header-2a-10-2d-00-6b' }),
...
function pixelDisc(ctx, x, y, radius, color, alpha = 1) {
  ctx.save(); ctx.globalAlpha = clamp(alpha); ctx.fillStyle = color;
  const r = Math.max(1, Math.round(radius));
  for (let row = -r; row <= r; row += 2) {
    const half = Math.round(Math.sqrt(Math.max(0, r * r - row * row)));
    ctx.fillRect(Math.round(x - half), Math.round(y + row), Math.max(2, half * 2), 2);
  }
  ctx.restore();
}

function stageRolePoint(sceneContext, role, fallback, width, height) {
  return {
    x: clamp(Number(sceneContext?.[`${role}X`] ?? fallback.x), 0, 100) / 100 * width,
    y: clamp(Number(sceneContext?.[`${role}Y`] ?? fallback.y), 0, 100) / 100 * height,
  };
}

function stagePointList(sceneContext, key, fallback, width, height) {
  const supplied = Array.isArray(sceneContext?.[key]) ? sceneContext[key] : [];
  return (supplied.length ? supplied : fallback).map((point) => ({
    x: clamp(Number(point.x ?? 50), 0, 100) / 100 * width,
    y: clamp(Number(point.y ?? 50), 0, 100) / 100 * height,
  }));
}

function drawMissile(ctx, frame, sceneContext = {}) {
  const width = Number(sceneContext.stageWidth ?? STAGE_WIDTH);
  const height = Number(sceneContext.stageHeight ?? STAGE_HEIGHT);
  const caster = stageRolePoint(sceneContext, 'caster', { x: 24, y: 45 }, width, height);
  const target = stageRolePoint(sceneContext, 'target', { x: 76, y: 50 }, width, height);
  const travel = easeInOut(segment(frame, 0, 42));
  const alpha = Math.min(segment(frame, 0, 5), 1 - segment(frame, 62, 72));
  const pointAt = (t) => ({ x: caster.x + (target.x - caster.x) * t, y: caster.y + (target.y - caster.y) * t });

  // SFC footage shows a white orb and broken circular echoes, not a rocket or targeting reticle.
  for (let echo = 4; echo >= 0; echo -= 1) {
    const t = clamp(travel - echo * .115);
    if (t <= 0 && echo) continue;
    const point = pointAt(t);
    const echoAlpha = alpha * (1 - echo * .16);
    if (echo === 0) {
      pixelDisc(ctx, point.x, point.y, 5 + Math.sin(frame * .7) * 1.2, '#ffffff', echoAlpha);
      pixelDisc(ctx, point.x - 1, point.y + 1, 2, '#fff8c8', echoAlpha);
    } else {
      const radius = 5 + echo * 2;
      ring(ctx, point.x, point.y, radius, echo % 2 ? '#fff8d0' : '#ffffff', 2, echoAlpha, -.8, Math.PI * 1.35);
      line(ctx, point.x - radius, point.y + 1, point.x - radius + 4, point.y + 3, '#ffffff', 2, echoAlpha);
    }
  }

  if (frame >= 42 && frame <= 61) {
    const flicker = Math.sin((frame - 42) * Math.PI * .5) > 0 ? '#ff9a30' : '#4b78d8';
    const pulse = Math.sin(segment(frame, 42, 61) * Math.PI);
    pixelDisc(ctx, target.x, target.y, 7 + pulse * 5, flicker, alpha * (.52 + pulse * .4));
    ring(ctx, target.x, target.y, 9 + pulse * 11, '#fff4c0', 3, alpha * pulse);
  }
}

function drawFlare(ctx, frame, sceneContext = {}) {
  const width = Number(sceneContext.stageWidth ?? STAGE_WIDTH);
  const height = Number(sceneContext.stageHeight ?? STAGE_HEIGHT);
  const caster = stageRolePoint(sceneContext, 'caster', { x: 76, y: 46 }, width, height);
  const target = stageRolePoint(sceneContext, 'target', { x: 25, y: 48 }, width, height);
  const finalFade = 1 - segment(frame, 384, 408);

  if (frame <= 55) {
    const aura = Math.min(segment(frame, 0, 12), 1 - segment(frame, 42, 55));
    for (let stroke = 0; stroke < 6; stroke += 1) {
      const side = stroke % 2 ? 1 : -1;
      const y = caster.y - 24 + stroke * 9;
      line(ctx, caster.x + side * 6, y + 7, caster.x + side * (13 + (stroke % 3) * 3), y, stroke % 3 ? '#7cc84a' : '#f8fff0', 3, aura);
      line(ctx, caster.x + side * (13 + (stroke % 3) * 3), y, caster.x + side * 8, y - 6, '#d8f870', 2, aura);
    }
  }

  if (frame >= 48 && frame <= 383) {
    const field = Math.min(segment(frame, 48, 72), 1 - segment(frame, 372, 384));
    stageFill(ctx, width, height, '#180000', .3 * field);
    stageFill(ctx, width, height, frame % 36 < 18 ? '#5a0800' : '#300000', .26 * field);
  }

  if (frame >= 48 && frame <= 330) {
    const fieldAlpha = Math.min(segment(frame, 48, 80), 1 - segment(frame, 310, 340)) * finalFade;
    for (let mote = 0; mote < 15; mote += 1) {
      const orbit = frame * (.012 + (mote % 4) * .003) + mote * 2.13;
      const collapse = Math.max(0, 1 - segment(frame, 120, 220) * .68);
      const radiusX = width * (.12 + (mote % 5) * .045) * collapse;
      const radiusY = height * (.08 + (mote % 4) * .035) * collapse;
      pixelDisc(ctx, target.x + Math.cos(orbit) * radiusX, target.y + Math.sin(orbit * 1.17) * radiusY, 2 + mote % 3, mote % 3 ? '#ff9b28' : '#fff0a0', fieldAlpha * (.5 + (mote % 4) * .1));
    }
  }

  if (frame >= 120 && frame <= 205) {
    const ignition = Math.sin(segment(frame, 120, 205) * Math.PI);
    pixelDisc(ctx, target.x, target.y, 8 + ignition * 11, '#ffe060', ignition);
    ring(ctx, target.x, target.y, 13 + ignition * 18, '#ff8a18', 4, ignition);
  }

  if (frame >= 190 && frame <= 285) {
    const grow = easeOut(segment(frame, 190, 236));
    const recede = 1 - easeInOut(segment(frame, 250, 285));
    const sphere = Math.min(grow, recede);
    const maxOuterRadius = Math.max(24, Math.min(target.x - 8, width - target.x - 8, target.y - 8, height - target.y - 8));
    const radius = 8 + sphere * Math.max(8, maxOuterRadius - 15);
    ring(ctx, target.x, target.y, radius + 7, '#ff7c10', 7, sphere);
    ring(ctx, target.x, target.y, Math.max(3, radius - 3), '#ffd030', 7, sphere);
    pixelDisc(ctx, target.x, target.y, Math.max(2, radius - 8), '#ffffff', sphere);
  }

  if (frame >= 265 && frame <= 371) {
    const after = Math.min(segment(frame, 265, 286), 1 - segment(frame, 350, 372));
    const pulse = .32 + .68 * Math.abs(Math.sin((frame - 265) * Math.PI / 18));
    stageFill(ctx, width, height, frame % 36 < 18 ? '#620900' : '#160000', after * .24 * pulse);
    ring(ctx, target.x, target.y, 18 + pulse * 27, '#ff9b20', 4, after * pulse);
    pixelDisc(ctx, target.x, target.y, 4 + pulse * 7, '#fff4b0', after);
  }

  if (frame >= 372) {
    const latch = Math.min(segment(frame, 372, 378), 1 - segment(frame, 384, 408));
    pixelDisc(ctx, target.x, target.y, 9 + latch * 9, '#ffd858', latch);
    ring(ctx, target.x, target.y, 16 + latch * 16, '#ff8418', 4, latch);
  }
}

function drawSerratedOval(ctx, x, y, radiusX, radiusY, color, alpha, offset = 0) {
  const points = [];
  for (let index = 0; index < 32; index += 1) {
    const angle = index / 32 * Math.PI * 2 + offset;
    const tooth = index % 2 ? 1 : .78;
    points.push({ x: x + Math.cos(angle) * radiusX * tooth, y: y + Math.sin(angle) * radiusY * tooth });
  }
  for (let index = 0; index < points.length; index += 1) {
    const from = points[index]; const to = points[(index + 1) % points.length];
    line(ctx, from.x, from.y, to.x, to.y, color, 3, alpha);
  }
}

function drawLevel5Death(ctx, frame, sceneContext = {}) {
  const width = Number(sceneContext.stageWidth ?? STAGE_WIDTH);
  const height = Number(sceneContext.stageHeight ?? STAGE_HEIGHT);
  const fallback = [{ x: 76, y: 30 }, { x: 76, y: 44 }, { x: 76, y: 58 }, { x: 76, y: 72 }];
  const targetFallback = Array.isArray(sceneContext.targets) && sceneContext.targets.length ? sceneContext.targets : fallback;
  const targets = stagePointList(sceneContext, 'hostileTargets', targetFallback, width, height);
  const explicitSuccess = Array.isArray(sceneContext.level5SuccessTargets);
  const successes = explicitSuccess
    ? stagePointList(sceneContext, 'level5SuccessTargets', [], width, height)
    : targets;
  const fadeOut = 1 - segment(frame, 144, 180);

  targets.forEach((target, index) => {
    const start = index * 8;
    const sweepIn = Math.max(index === 0 && frame === 0 ? .08 : 0, segment(frame, start, start + 22));
    const sweep = Math.min(sweepIn, 1 - segment(frame, 144, 180)) * fadeOut;
    if (sweep <= 0) return;
    const cycle = Math.floor(Math.max(0, frame - start) / 9);
    const centerY = target.y + ((cycle % 3) - 1) * 13;
    drawSerratedOval(ctx, target.x, centerY, 18 + cycle % 2 * 3, 28, '#48b838', sweep, frame * .025);
    drawSerratedOval(ctx, target.x, centerY, 14 + cycle % 2 * 2, 23, '#f0e840', sweep * .92, -frame * .02);
    drawSerratedOval(ctx, target.x, centerY, 10, 18, '#f08820', sweep * .8, frame * .018);
    pixelDisc(ctx, target.x, centerY, 4, '#fff8a0', sweep);
  });

  if (frame >= 92 && frame <= 159) {
    const burstAlpha = Math.min(segment(frame, 92, 112), 1 - segment(frame, 140, 160));
    successes.forEach((target, targetIndex) => {
      for (let puff = 0; puff < 9; puff += 1) {
        const angle = puff * Math.PI * 2 / 9 + targetIndex * .41;
        const distance = 5 + (puff % 3) * 9 + segment(frame, 92, 140) * 12;
        const color = puff % 3 === 0 ? '#fff8c8' : puff % 3 === 1 ? '#f8d058' : '#e88028';
        pixelDisc(ctx, target.x + Math.cos(angle) * distance, target.y + Math.sin(angle) * distance * .72, 5 + puff % 3 * 2, color, burstAlpha);
      }
    });
  }
}
...
function drawReferenceCastBurst(ctx, point, progress, alpha = 1) {
  for (let ray = 0; ray < 12; ray += 1) {
    const angle = ray * Math.PI * 2 / 12 + (ray % 2 ? .09 : -.05);
    const inner = 9 + progress * 3;
    const outer = 18 + progress * 13 + (ray % 3) * 3;
    line(ctx, point.x + Math.cos(angle) * inner, point.y + Math.sin(angle) * inner, point.x + Math.cos(angle) * outer, point.y + Math.sin(angle) * outer, ray % 3 ? '#e8e0c8' : '#ffffff', ray % 2 ? 2 : 3, alpha);
  }
}

function drawHasteReference(ctx, frame, sceneContext = {}) {
  const width = Number(sceneContext.stageWidth ?? STAGE_WIDTH);
  const height = Number(sceneContext.stageHeight ?? STAGE_HEIGHT);
  const fallback = [{ x: Number(sceneContext.targetX ?? 76), y: Number(sceneContext.targetY ?? 50) }];
  const targets = stagePointList(sceneContext, 'targets', fallback, width, height);
  const alpha = Math.min(segment(frame, 0, 6), 1 - segment(frame, 73, 96));
  const seed = Math.min(segment(frame, 0, 17), 1 - segment(frame, 39, 48));
  const orb = Math.min(segment(frame, 38, 54), 1 - segment(frame, 72, 96));

  targets.forEach((target, targetIndex) => {
    drawReferenceCastBurst(ctx, target, easeOut(segment(frame, 0, 28)), alpha * seed);
    if (orb <= 0) return;
    const breathe = 1 + Math.sin((frame + targetIndex * 3) * Math.PI / 8) * .08;
    pixelDisc(ctx, target.x, target.y, 13 * breathe, '#f07818', alpha * orb);
    pixelDisc(ctx, target.x - 2, target.y - 2, 9 * breathe, '#ffd838', alpha * orb);
    ring(ctx, target.x, target.y, 15 * breathe, '#fff0a0', 3, alpha * orb);
    const satellites = [[-23, -12, 6], [2, -27, 6], [26, -8, 6], [17, 23, 7]];
    satellites.forEach(([dx, dy, radius], satelliteIndex) => {
      const pulse = 1 + Math.sin(frame * .31 + satelliteIndex * 1.7) * .12;
      pixelDisc(ctx, target.x + dx, target.y + dy, radius * pulse, '#f08018', alpha * orb);
      pixelDisc(ctx, target.x + dx - 1, target.y + dy - 1, Math.max(2, radius * pulse - 3), '#ffd838', alpha * orb);
      ring(ctx, target.x + dx, target.y + dy, radius * pulse + 2, '#fff0a0', 2, alpha * orb);
    });
  });
}

function drawGravityReference(ctx, frame, sceneContext = {}) {
  const width = Number(sceneContext.stageWidth ?? STAGE_WIDTH);
  const height = Number(sceneContext.stageHeight ?? STAGE_HEIGHT);
  const caster = stageRolePoint(sceneContext, 'caster', { x: 76, y: 50 }, width, height);
  const target = stageRolePoint(sceneContext, 'target', { x: 24, y: 50 }, width, height);
  const alpha = Math.min(segment(frame, 0, 6), 1 - segment(frame, 89, 108));
  const cast = Math.min(segment(frame, 0, 13), 1 - segment(frame, 26, 35));
  drawReferenceCastBurst(ctx, caster, easeOut(segment(frame, 0, 22)), alpha * cast);

  const travel = easeInOut(segment(frame, 20, 66));
  const orbX = caster.x + (target.x - caster.x) * travel;
  const orbY = caster.y + (target.y - caster.y) * travel;
  const capture = frame < 66 ? 1 : 1 - segment(frame, 89, 108);
  pixelDisc(ctx, orbX, orbY, 16 + Math.sin(frame * .24) * 2, '#202040', alpha * capture);
  ring(ctx, orbX, orbY, 18 + Math.sin(frame * .24) * 2, '#7898d8', 3, alpha * capture);
  ring(ctx, orbX, orbY, 21 + Math.sin(frame * .19) * 2, '#b8d8f0', 2, alpha * capture * .72);

  if (frame >= 62) {
    const crack = Math.min(segment(frame, 62, 74), 1 - segment(frame, 89, 108));
    const branches = [
      [[-10, -10], [-3, -3], [-8, 4], [0, 9]],
      [[7, -13], [2, -5], [8, 1], [3, 12]],
      [[-15, 1], [-7, 2], [-2, 8], [8, 10]],
    ...
    branches.forEach((points) => {
      for (let index = 1; index < points.length; index += 1) {
        line(ctx, target.x + points[index - 1][0], target.y + points[index - 1][1], target.x + points[index][0], target.y + points[index][1], '#ffffff', 2, alpha * crack);
      }
    });
  }
}

const METEOR_REFERENCE_TRACKS = Object.freeze([
  { start: 56, impact: 98, fromX: .94, fromY: .04, offsetX: -18, offsetY: -12 },
  { start: 80, impact: 126, fromX: .08, fromY: .08, offsetX: 18, offsetY: 7 },
  { start: 104, impact: 154, fromX: .72, fromY: -.05, offsetX: -5, offsetY: 16 },
  { start: 132, impact: 182, fromX: .28, fromY: .02, offsetX: 13, offsetY: -18 },
  { start: 64, impact: 112, fromX: .54, fromY: -.08, offsetX: 28, offsetY: -22 },
  { start: 116, impact: 170, fromX: 1.05, fromY: .16, offsetX: -29, offsetY: 18 },
]);

function drawMeteorReference(ctx, frame, sceneContext = {}) {
  const width = Number(sceneContext.stageWidth ?? STAGE_WIDTH);
  const height = Number(sceneContext.stageHeight ?? STAGE_HEIGHT);
  const caster = stageRolePoint(sceneContext, 'caster', { x: 76, y: 50 }, width, height);
  const target = stageRolePoint(sceneContext, 'target', { x: 24, y: 50 }, width, height);
  const castAlpha = Math.min(segment(frame, 0, 10), 1 - segment(frame, 28, 38));
  drawReferenceCastBurst(ctx, caster, easeOut(segment(frame, 0, 31)), castAlpha);

  const field = Math.min(segment(frame, 32, 48), 1 - segment(frame, 228, 252));
  stageFill(ctx, width, height, frame % 22 < 11 ? '#180000' : '#380400', field * .68);
  for (let band = 0; band < 9; band += 1) {
    const y = ((band * 47 + frame * (band % 3 + 1)) % (height + 54)) - 27;
    const wave = Math.sin(frame * .07 + band * 1.9) * 18;
    line(ctx, -20, y, width * .34 + wave, y - 7, band % 3 ? '#580b08' : '#9a1b0d', band % 2 ? 5 : 9, field * .72);
    line(ctx, width * .56 - wave, y + 5, width + 20, y - 2, band % 2 ? '#2b0000' : '#781008', band % 3 ? 6 : 10, field * .64);
  }

  METEOR_REFERENCE_TRACKS.forEach((track, trackIndex) => {
    const destination = { x: target.x + track.offsetX, y: target.y + track.offsetY };
    if (frame >= track.start && frame <= track.impact) {
      const travel = easeInOut(segment(frame, track.start, track.impact));
      const x = track.fromX * width + (destination.x - track.fromX * width) * travel;
      const y = track.fromY * height + (destination.y - track.fromY * height) * travel;
      for (let tail = 5; tail >= 1; tail -= 1) {
        const trailX = x - (destination.x - track.fromX * width) / Math.max(42, track.impact - track.start) * tail * 3;
        const trailY = y - (destination.y - track.fromY * height) / Math.max(42, track.impact - track.start) * tail * 3;
        pixelDisc(ctx, trailX, trailY, Math.max(2, 7 - tail), tail % 2 ? '#d83810' : '#ff8c18', field * (1 - tail * .12));
      }
      pixelDisc(ctx, x, y, 8 + trackIndex % 3 * 2, '#dc3410', field);
      pixelDisc(ctx, x - 1, y - 1, 5 + trackIndex % 2 * 2, '#ff9418', field);
      pixelDisc(ctx, x - 2, y - 2, 2 + trackIndex % 2, '#ffe060', field);
    }
    if (frame >= track.impact && frame <= track.impact + 24) {
      const blast = Math.min(segment(frame, track.impact - 4, track.impact + 6), 1 - segment(frame, track.impact + 7, track.impact + 24));
      pixelDisc(ctx, destination.x, destination.y, 6 + blast * 15, '#ff7c10', field * blast);
      pixelDisc(ctx, destination.x - 1, destination.y - 1, 3 + blast * 9, '#ffe048', field * blast);
      ring(ctx, destination.x, destination.y, 8 + blast * 22, trackIndex % 2 ? '#ff3810' : '#ff9c20', 4, field * blast);
    }
  });
}

function drawReturnReference(ctx, frame, sceneContext = {}) {
  const width = Number(sceneContext.stageWidth ?? STAGE_WIDTH);
  const height = Number(sceneContext.stageHeight ?? STAGE_HEIGHT);
  const caster = stageRolePoint(sceneContext, 'caster', { x: 76, y: 50 }, width, height);
  const target = stageRolePoint(sceneContext, 'target', { x: 24, y: 50 }, width, height);
  const cast = Math.min(segment(frame, 0, 12), 1 - segment(frame, 25, 34));
  drawReferenceCastBurst(ctx, caster, easeOut(segment(frame, 0, 27)), cast);

  const fracture = Math.min(segment(frame, 28, 62), 1 - segment(frame, 121, 150));
  const blockSize = 3 + Math.floor(segment(frame, 28, 92) * 8);
  const silhouette = [
    [-1, -4], [0, -4], [-2, -3], [-1, -3], [0, -3], [1, -3],
    [-2, -2], [-1, -2], [0, -2], [1, -2], [2, -2],
    [-2, -1], [-1, -1], [0, -1], [1, -1], [2, -1],
    [-2, 0], [-1, 0], [0, 0], [1, 0], [2, 0],
    [-1, 1], [0, 1], [1, 1], [-2, 2], [1, 2], [-2, 3], [2, 3],
  ];
  const colors = ['#e0d0a8', '#807868', '#282820', '#f0e8c8', '#5a5448'];
  ctx.save();
  ctx.globalAlpha = clamp(fracture);
  silhouette.forEach(([gridX, gridY], index) => {
    const expansion = 1 + segment(frame, 52, 110) * (1.7 + index % 3 * .22);
    const x = Math.round(target.x + gridX * blockSize * expansion + (index % 2 ? 1 : -1) * segment(frame, 74, 110) * 9);
    const y = Math.round(target.y + gridY * blockSize * expansion + ((index % 3) - 1) * segment(frame, 74, 110) * 6);
    ctx.fillStyle = colors[index % colors.length];
    ctx.fillRect(x, y, blockSize, blockSize);
  });
  ctx.restore();

  const darkness = Math.min(segment(frame, 64, 110), 1 - segment(frame, 130, 168));
  stageFill(ctx, width, height, '#000000', darkness * .94);
  if (frame >= 92) {
    const reset = Math.min(segment(frame, 92, 110), 1 - segment(frame, 130, 168));
    for (let row = 0; row < 6; row += 1) {
      const size = 7 + row % 3 * 4;
      const x = target.x + ((row * 29 + frame * 3) % 80) - 40;
      const y = target.y + ((row * 17 - frame * 2) % 96) - 48;
      ctx.save(); ctx.globalAlpha = clamp(reset * .76); ctx.fillStyle = row % 2 ? '#d8d0b0' : '#403c34'; ctx.fillRect(Math.round(x), Math.round(y), size, size); ctx.restore();
    }
  }
}
...
function renderScene(ctx, sceneId, frame, sceneContext = {}) {
  ...
  if (sceneId === 'haste') return drawHasteReference(ctx, frame, sceneContext);
  if (['slow', 'stop'].includes(sceneId)) return drawTime(ctx, frame, sceneId);
  if (sceneId === 'comet') return drawComet(ctx, frame, false);
  if (sceneId === 'meteor') return drawMeteorReference(ctx, frame, sceneContext);
  if (sceneId === 'gravity') return drawGravityReference(ctx, frame, sceneContext);
  if (sceneId === 'graviga') return drawGravity(ctx, frame, 2);
  if (sceneId === 'return') return drawReturnReference(ctx, frame, sceneContext);
  if (sceneId === 'missile') return drawMissile(ctx, frame, sceneContext);
  if (sceneId === 'flare') return drawFlare(ctx, frame, sceneContext);
  if (sceneId === 'level-5-death') return drawLevel5Death(ctx, frame, sceneContext);
  ...
  if (sceneId === 'hastega') return drawHasteReference(ctx, frame, sceneContext);
